...
namespace Shadows2D
{
    ...
    public class ShadowMapResolver
    {
        ...
        public void ResolveShadowsTransparent(ShadowCasterMap shadowCasterMap, LightSource resultLight, PostEffect postEffect, Vector2 newPosition)
        {
            resultLight.Position = newPosition;
            this.ResolveShadowsTransparent(shadowCasterMap, resultLight, postEffect, 1f);
        }
        ...
        public void ResolveShadowsTransparent(ShadowCasterMap shadowCasterMap, LightSource resultLight, PostEffect postEffect, float distanceMod)
        {
            BlendState backupBlendState = graphicsDevice.BlendState;
            graphicsDevice.BlendState = BlendState.Opaque;

            this.ExecuteTechniqueDistortAndComputeDistanceTransparent(shadowCasterMap, resultLight, shadowMapDistorted, "DistortAndComputeDistances");
            ...
            switch (postEffect)
            {
                case PostEffect.LinearAttenuation:
                    {
                        this.ExecuteTechniqueDrawShadowsTransparent(shadowsRT, "DrawShadowsLinearAttenuation", this.shadowMapDigested);
                    }
                    break;

                case PostEffect.LinearAttenuation_BlurHigh:
                    {
                        this.ExecuteTechniqueDrawShadowsTransparent(shadowsRT, "DrawShadowsNoAttenuationPreBlur", this.shadowMapDigested);
                        this.ExecuteTechniqueBlurHTransparent(shadowsRT, processedShadowsRT, "BlurHorizontallyHigh");
                        this.ExecuteTechniqueBlurVTransparent(processedShadowsRT, resultLight.PrintedLight, "BlurVerticallyHighLinearAttenuation");
                    }
                    break;
                ...
                default: //NOFX
                     ...
                     */
                default:
                    throw new NotImplementedException("I didn't implement this. Yell at me. -Elliot");
            }
            graphicsDevice.BlendState = backupBlendState;
        }
        ...
        private void ExecuteTechniqueDistortAndComputeDistance(ShadowCasterMap shadowCasterMap, LightSource light, RenderTarget2D destination, string techniqueName)
        {
            ...
            this.currenteRenderTargetSize.X = destination.Width;
            this.currenteRenderTargetSize.Y = destination.Height;
        ...
        private void ExecuteTechniqueDistortAndComputeDistanceTransparent(ShadowCasterMap shadowCasterMap, LightSource light, RenderTarget2D destination, string techniqueName)
        {
            graphicsDevice.SetRenderTarget(destination);
        ...
        private void ExecuteTechniqueDrawShadowsTransparent(RenderTarget2D destination, string techniqueName, Texture2D shadowMap)
        {
            Vector2 renderTargetSize = new Vector2(this.shadowMapResolverSize, this.shadowMapResolverSize);
            graphicsDevice.SetRenderTarget(destination);
            graphicsDevice.Clear(Color.Transparent);
        ...
        private void ExecuteTechniqueBlurHTransparent(Texture2D source, RenderTarget2D destination, string techniqueName)
        {
            Vector2 renderTargetSize = new Vector2(this.shadowMapResolverSize, this.shadowMapResolverSize);
            graphicsDevice.SetRenderTarget(destination);
            graphicsDevice.Clear(Color.Transparent);
        ...
        private void ExecuteTechniqueBlurVTransparent(Texture2D source, RenderTarget2D destination, string techniqueName)
        {
            Vector2 renderTargetSize = new Vector2(this.shadowMapResolverSize, this.shadowMapResolverSize);
            graphicsDevice.SetRenderTarget(destination);
            graphicsDevice.Clear(Color.Transparent);
